...
import { ensureSession, firebaseReady, listenClasses, listenTasks, saveClass, saveTask, submitEntry } from './firebase'
...
function App() {
  ...
  const login = async role => {
    setUserType(role)
    try { const user = await ensureSession(role, USERS[role].name); setFirebaseUser(user ? { uid:user.uid, name:USERS[role].name } : null) }
...
function TaskWorkspace({ task, student, onBack, onUpdate }) {
  ...
  const submit = async () => { if(!entryFile){setFileError('제출할 엔트리 파일을 선택해 주세요.');return}if(!firebaseReady||!student){setFileError('Firebase 연결 후 제출할 수 있습니다. .env 설정을 확인해 주세요.');return}try{setUploading(true);await submitEntry({taskId:task.id,file:entryFile,explanation:answer,student});setResult('correct');onUpdate({status:'done',attempts:(task.attempts||0)+1})}catch(error){setFileError(`제출 실패: ${error.message}`)}finally{setUploading(false)} }
...
function AdminPage({ active, classes, onClassAdded }) {
