...
function TaskWorkspace({ task, student, onBack, onUpdate }) {
  ...
  return <div className="workspace-page">
    ...
    <div className="workspace-grid">
      <section className="problem-panel panel"><div className="panel-heading"><span>과제 안내</span><div className="step-dots"><i className="on"></i><i></i><i></i></div></div><div className="problem-body"><div className="problem-no">과제 설명</div><div className="teacher-description">{task.description?.trim()||'교사가 작성한 과제 설명이 없습니다.'}</div>{task.hint&&<div className="hint-box"><Lightbulb size={20}/><div><b>생각 열기</b><p>{task.hint}</p></div></div>}{task.codeFileUrl&&<><div className="submission-heading"><b>과제 코드</b><p>이어서 작업할 엔트리 기본 코드를 내려받으세요.</p></div><button type="button" className="secondary" disabled={codeDownloading} onClick={downloadCode}><Download size={18}/>{codeDownloading?'다운로드 중...':`${task.codeFileName||'과제 코드'} 다운로드`}</button></>}<div className="submission-heading"><b>과제 파일 제출</b><p>완성한 엔트리 프로젝트 파일을 올려 주세요.</p></div><label className={`file-drop ${fileError?'has-error':''} ${entryDragging?'is-dragging':''} ${result==='correct'?'is-locked':''}`} onDragEnter={e=>{if(result==='correct')return;e.preventDefault();setEntryDragging(true)}} onDragOver={e=>{if(result==='correct')return;e.preventDefault();setEntryDragging(true)}} onDragLeave={()=>setEntryDragging(false)} onDrop={e=>{e.preventDefault();setEntryDragging(false);if(result==='correct')return;chooseFile(e.dataTransfer.files?.[0])}}><input type="file" accept=".ent,application/octet-stream" disabled={result==='correct'} onChange={e=>chooseFile(e.target.files?.[0])}/><div className="file-icon">{entryFile?<FileCode2/>:<Upload/>}</div><div><b>{entryDragging?'여기에 놓아 주세요':entryFile?entryFile.name:'엔트리 파일 업로드'}</b><p>{entryDragging?'.ent 파일을 놓으면 선택됩니다.':entryFile?`${(entryFile.size/1024).toFixed(1)} KB · 다른 파일 선택 가능`:'.ent 파일을 선택하거나 여기에 끌어다 놓으세요.'}</p></div>{entryFile&&result!=='correct'&&<button type="button" className="file-remove" aria-label="첨부 파일 삭제" onClick={e=>{e.preventDefault();e.stopPropagation();removeFile()}}><X size={15}/></button>}</label>{fileError&&<p className="field-error">{fileError}</p>}{result && <div className={`result ${result}`}>
        {result === 'correct' ? <><CheckCircle2/><div><b>정답</b></div></> : <><XCircle/><div><b>다시 수정해보세요.</b></div></>}
      </div>}{result==='correct'?<button className="primary submit" onClick={onBack}>확인 <Check size={18}/></button>:<button className="primary submit" onClick={submit} disabled={uploading}>{uploading?'채점하는 중...':result==='wrong'?'수정해서 다시 제출':'과제 제출하기'}{!uploading&&<ArrowRight size={18}/>}</button>}</div></section>
